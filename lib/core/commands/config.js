...
async function handleConfigCommand(bot, params, reply) {
    ...
    if (secondCmd === 'detail') {
        ...
        const detail = `
〓 AnemoBot 详细配置 〓
登录模式: ${exports.ModeMap[config_1.AnemoConf.login_mode] ?? '未知'}
设备锁模式: ${exports.ModeMap[config_1.AnemoConf.device_mode] ?? '未知'}
...
接收自己消息: ${config_1.AnemoConf.oicq_config.ignore_self}
`.trim();
        return reply(detail);
    }
    ...
    if (secondCmd === 'me') {
        if (thirdCmd === 'on') {
            config_1.AnemoConf.oicq_config.ignore_self = true;
            if ((0, config_1.saveAnemoConf)()) {
                reply('〓 接收自己消息开启 〓');
            }
        }
        else if (thirdCmd === 'off') {
            config_1.AnemoConf.oicq_config.ignore_self = false;
            if ((0, config_1.saveAnemoConf)()) {
                reply('〓 接收自己消息关闭 〓');
            }
        }
