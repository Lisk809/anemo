...
const AboutText = `
〓 关于 Anemo 〓
Anemo 是一个开源、轻量、跨平台、注重体验、开发者友好的 QQ 机器人框架，基于 Node.js 和 oicq v2 构建。
使用文档: https://anemobot.cn/
`.trim();
...
async function handleAnemoCommand(event, bot, AnemoConf) {
    ...
    if (cmd === 'exit') {
        await event.reply('〓 Anemo 进程已停止 〓');
        utils_1.notice.success('框架进程已由管理员通过 /exit 消息指令退出');
        child_process.exec('anemo stop',(err)=>{
            if(err){
                //have problem
                event.reply('〓 框架退出失败 〓');
            }
    ...
    if (cmd === 'reboot') {
        await event.reply('〓 Anemo 进程重启中... 〓');
        utils_1.notice.success('框架进程重启中...');
        let cmd = 'anemo stop && anemo deploy';
        child_process.exec('anemo list',(error,stdout)=>{
            if(error||stdout.includes('stopped')){
                cmd = 'anemo stop && anemo start';
            }
        ...
        try {
            ...
            if (upInfo) {
                const info = Object.entries(upInfo)
                    .map(([k, v]) => `${k.replace('anemobot-plugin-', '插件: ')} => ${v.replace('^', '')}`)
                    .join('\n');
                const msg = info
                    ? `〓 更新成功，内容如下 〓\n${info}\ntip: 需要重启框架才能生效`
                    : '〓 已是最新版本 〓';
            ...
            _src_1.AnemoLogger.error((0, utils_1.stringifyError)(e));
