...
const { PluginDataDir } = require("@anemo/core/lib/core/path");
exports.PluginText = `
〓 Anemo 插件 〓
/plugin list
/plugin add <name>
...
`.trim();
    ...
    if (secondCmd === 'onall') {
        ...
        ps.forEach(async (path, i) => {
            ...
            const res = await (0, plugin_1.enablePlugin)(bot, config_1.AnemoConf, path);
            if (res === true) {
                count++;
            }
            else {
    ...
    if (secondCmd === 'offall') {
        ...
        Array.from(start_1.plugins.entries()).forEach(async ([pname, plugin], i) => {
            ...
            if (targetPluginPath) {
                const res = await (0, plugin_1.disablePlugin)(bot, config_1.AnemoConf, plugin, targetPluginPath);
                if (res !== true) {
                    reply(`〓 ${pname} 禁用失败 〓\n${res}`);
                }
    ...
    if (secondCmd === 'reloadall') {
        ...
        Array.from(start_1.plugins.entries()).forEach(async ([pname, plugin], i) => {
            ...
            if (targetPluginPath) {
                const res = await (0, plugin_1.disablePlugin)(bot, config_1.AnemoConf, plugin, targetPluginPath);
                const res_1 = await (0, plugin_1.enablePlugin)(bot, config_1.AnemoConf, targetPluginPath);
    ...
    if (secondCmd === 'update') {
        ...
        const name = pname ? `${pname} ` : '';
        try {
            const upInfo = await (0, utils_1.update)(`anemo-plugin-${pname || '*'}`);
            if (upInfo) {
                const info = Object.entries(upInfo)
                    .map(([k, v]) => `${k.replace('anemo-plugin-', '插件: ')} => ${v.replace('^', '')}`)
                    .join('\n');
            ...
            logger_1.AnemoLogger.error((0, utils_1.stringifyError)(e));
    ...
    if (secondCmd === 'on') {
        ...
        const res = await (0, plugin_1.enablePlugin)(bot, config_1.AnemoConf, targetPluginPath);
        if (res === true) {
            if ((0, config_1.saveAnemoConf)()) {
    ...
    if (secondCmd === 'reload') {
        ...
        let res = false;
        if (!plugin) {
            res = await (0, plugin_1.enablePlugin)(bot, config_1.AnemoConf, targetPluginPath);
        }
        else {
            res = await (0, plugin_1.disablePlugin)(bot, config_1.AnemoConf, plugin, targetPluginPath);
            res = res && (await (0, plugin_1.enablePlugin)(bot, config_1.AnemoConf, targetPluginPath));
        }
        if (res === true) {
            if ((0, config_1.saveAnemoConf)()) {
    ...
    if (secondCmd === 'add') {
        ...
        let shortName = pname;
        if (/^anemo-plugin-/i.test(shortName)) {
            shortName = shortName.replace(/^anemo-plugin-/i, '');
        }
        reply(`〓 正在安装 ${pname} 〓`);
        try {
            if (await (0, utils_1.install)(`anemo-plugin-${shortName}`)) {
