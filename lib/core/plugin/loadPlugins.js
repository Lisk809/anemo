...
async function loadPlugins(bot, lskConf) {
    // 检索本地所有插件（node_modukles 里 `lskbot-plugin-` 开头的插件 和 plugins 下的插件）
    ...
    for (const pluginPath of plugins) {
        ...
        // 跳过未设置启用的插件
        if (!lskConf.plugins.includes(pluginName)) {
            continue;
        }
        const isOK = await (0, enablePlugin_1.enablePlugin)(bot, lskConf, pluginPath);
        // 启用成功时，启用插件数加一
        if (isOK)
            cnt++;
    }
