...
class LskPlugin extends node_events_1.default {
    /**
     * LskBot 插件类
     *
    ...
    constructor(name, version, conf) {
        ...
        this.config = conf ?? {};
        this.debug('create LskPlugin instance');
        if (!conf?.debug) {
            // 正式环境下确保插件的数据目录存在
    ...
    /**
     * 抛出一个 LskBot 插件标准错误，会被框架捕获
     *
     * @param {string} message 错误信息
     */
    throwPluginError(message) {
        throw new pluginError_1.LskPluginError(this.name, message);
    }
    /**
    ...
    /**
     * **插件请勿调用**，LskBot 框架调用此函数启用插件
     * @param {Client} bot oicq 机器人实例
     * @param {AdminArray} admins 框架管理员列表
     * @return {Promise<LskPlugin>} 插件实例
     */
    async mountLskBotClient(bot, admins) {
        this.debug('mountLskBotClient');
        // 挂载 Bot
        this.bot = bot;
        // 初始化管理员
        this._admins = [...admins];
        // 监听框架管理变动
        bot.on('lsk.admins', this.adminChangeHandler);
        try {
            this.debug('_mounted');
            // 调用 onMounted 挂载的函数
    ...
    /**
     * **插件请勿调用**，LskBot 框架调用此函数禁用插件
     * @param {Client} bot oicq 机器人实例
     * @param {AdminArray} admins 框架管理员列表
     */
    async unmountLskBotClient(bot, admins) {
        this.debug('unmountLskBotClient');
        // 取消监听框架管理变动
        bot.off('lsk.admins', this.adminChangeHandler);
        try {
